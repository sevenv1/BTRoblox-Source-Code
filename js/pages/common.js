...
const robloxLinkify = target => {
	...
	injectScript.call("linkify", null, className)
...
const redirectEvents = (from, to) => {
	...
	injectScript.call("redirectEvents", null, `[data-redirect-events="${redirectIndex}"]`, `[data-redirect-events="${redirectIndex + 1}"]`)
}

//

const initReactFriends = () => {
	injectScript.call("initReactFriends", null)
}

const initReactRobuxToCash = () => {
	if(!RobuxToCash.isEnabled()) { return }
	
	injectScript.call("initReactRobuxToCash", null)
}
...
const initDownloadButton = async (assetId, assetTypeId, isBundle) => {
	...
	if(downloadButton.dataset.toggle) {
		setTimeout(() => { // a bit ugly, but eh
			injectScript.call("setupPopovers", null)
		}, 0)
	}
...
pageInit.www = () => {
	...
	injectScript.call("addBTRSettings", null)
	...
	if(SETTINGS.get("general.fixFirefoxLocalStorageIssue")) {
		injectScript.call("fixFirefoxLocalStorageIssue", null)
	}
	
	if(SETTINGS.get("general.cacheRobuxAmount")) {
		injectScript.call("cacheRobuxAmount", null)
	}
	
	if(SETTINGS.get("general.higherRobuxPrecision")) {
		injectScript.call("higherRobuxPrecision", null)
	}
	...
	if(SETTINGS.get("groups.shoutAlerts") && SETTINGS.get("groups.shoutAlertsInNotifStream")) {
		...
		injectScript.call("shoutAlerts", null)
	}
	
	if(SETTINGS.get("home.hideFriendActivity")) {
		injectScript.call("hideFriendActivity", null)
	}
	...
	if(SETTINGS.get("general.hideChat")) {
		...
		if(SETTINGS.get("general.smallChatButton")) {
			...
			injectScript.call("smallChatButton", null)
		}
	}
}